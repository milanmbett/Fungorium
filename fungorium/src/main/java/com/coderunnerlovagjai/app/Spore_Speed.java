...
public class Spore_Speed extends Basic_Spore
{
    ...
    public Spore_Speed(Tecton_Class targetTecton, Player player)
    {
        timeToLive = 3;
        tecton = targetTecton;
        tecton.set_Spore(this);
        owner=player;
        ID = "Spore_Speed" + Integer.toString(owner.getGame().getPlane().SporeCollection.size());
        ...
        SPORE_SPEED_LOGGER.log(Level.forName("ADD", 403), "Spore_Speed: "+ID+ " added to SporeCollection! SporeCollection size: " + owner.getGame().getPlane().SporeCollection.size());
    }
    @Override
