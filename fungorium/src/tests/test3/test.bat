...
REM (Assuming .env is one level above test3)
...
REM Run the Java program with parameter "--test 3" and capture output.
for /f "delims=" %%a in ('java -jar "%JAR_FILE%" --test 3 ^< "%INPUT_FILE%"') do (
    set "OUTPUT=%%a"
)

REM Define expected substrings to search for:
...
REM Check each pattern in the output using findstr.
